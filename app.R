...
# pathShapeFile <- paste0(rootDir, "REGC2013_HD_Clipped(WGS84).shp") # FIXME: takes ages to load - stalls
...
server <- function(input, output, session) {
 ...
 ################################################################ 
  ...
  createMainMap <- function() {
    renderLeaflet({
      leaflet() %>%
        setView(lng = 173.41, lat = -41.3, zoom = 5) %>%
        addTiles()  %>%
